...
受注編集時の再計算を **Docker環境で実際に操作して動作を確認した**（`docs/verification.md`
...
**未確認の項目**: PHP 7.4／8.0／8.1 系や WordPress 6.6 以外のバージョンでの動作は
未確認。
...
- [x] カート画面・購入確認画面・受注データの3箇所で割引が整合（Docker環境で操作して確認済み。
...
（Docker環境での確認値: 正しくは -¥500 のところ **-¥1,000**、さらにそこから数量変更を
...
Docker環境で実際に操作して確認済み。

発生条件の解析・確認手順の詳細は、`docs/design-notes.md` の
