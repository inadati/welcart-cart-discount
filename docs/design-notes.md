...
また、実装直前の再調査（実装計画タスク5）およびDocker環境での検証（タスク15）で判明した、
...
- **$discount の性質**: このフックが呼ばれる時点の `$discount` は、Welcart が
  ...
  他の割引と共存しつつ二重計上を起こさない。この前提はDocker環境での検証（タスク15）の
...
- **$discount の性質（タスク15のDocker環境での検証で判明・第1回修正）**: `usces_order_discount`
  ...
  **現在表示されている値**（＝前回保存時に本プラグインが書き込んだ割引額を含む）
  ...
  割引が二重・三重に計上される不具合があった（Docker環境で ¥30,000/¥2,000引き の受注を
...
- **第2回修正（自プラグイン注入額のみを差し戻す精緻化、コミット
  ...
  **【誤った前提とその訂正】** 当初、前回本プラグインが注入した割引額を記録する先として
  ...
  検証（Docker + Playwright、詳細は `docs/verification.md` 「8. 受注再計算の
  ...
  より前に作成された受注）に対する挙動には既知の限界が残ることもDocker環境で確認した。
  ...
  記録されることをDocker環境で確認した。さらにこの新規受注を管理画面で数量変更・
  「Recalculation」した際も、正しい段（-¥500）に切り替わり、複数回クリックしても
  値が変わらないことを確認した。詳細は `docs/verification.md`
  「9. 新規受注（フロント購入フロー）での受注メタ自動記録のDocker環境での確認」を参照。
...
メタが存在しない受注）に対して既知の限界が残る。検証環境（Docker 上の実際の受注データ、
ID 1000）で以下を確認した。
...
変更して再計算する具体的な操作）まではDocker環境で検証しておらず、-¥2,500 という数値は
...
そのまま**数量を10・¥30,000に変更して「Recalculation」を押す）をDocker環境で行い、
...
続けるため `-500 - 2000 = -2500`）がDocker環境での実際の挙動と正確に一致することが確認できた。
...
保存する」という手順そのものがDocker環境で機能することは未検証だった。3周目修正で、
...
1. **`e2e/bin/env-up.sh` の環境構築ロジック自体は健全**であり、「まっさらな状態
   からでも1コマンドで用意できる」という設計目標はDocker環境で裏付けられた（上記の
   実測時間・分岐の通過を参照）。
