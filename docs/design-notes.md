...
同様の理由により、タスク10「全体通しの確認」で計画されていた「`composer e2e:down` で
まっさらな状態から再構築する」検証は、今回**あえて実行しなかった**。検証環境
（`docker-wordpress-1` 等）は本ドキュメント作成時点で4時間以上起動し続けており、
`docs/verification.md` に記録した上記の既存受注群を保持している。`docker/setup.sh` は
WordPress・Welcart・商品データを冪等に再現できるが、これらの受注は何周もの手動ブラウザ
操作でしか再現できないため、`composer e2e:down` によるボリューム破棄はこの検証データを
永久に失わせる不可逆な操作になる。そのため、設計書・実装計画が想定する「まっさらな状態
からの通し確認」の代わりに、既存環境を再利用する `composer e2e:up && composer e2e` で
代替検証した（11件PASS、2回実行しどちらも約42秒。2回目も同程度の時間だったのは、
そもそも1回目の時点で環境がすでに起動済みだったため）。`e2e/bin/env-up.sh` /
`env-down.sh` のロジック自体（コンテナ未起動時は起動する、商品不足時は再投入する等）は
コードレビューで妥当性を確認済みであり、実際に破棄→再構築する検証は今回のスコープでは
危険側に倒して見送った、という判断の記録である。
